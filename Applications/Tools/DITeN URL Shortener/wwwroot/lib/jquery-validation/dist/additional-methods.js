	...
	}(function($)
	{
			...
			, function(value
			           ...
			           , param)
				{
					...
					if ($(element)
						...
						"file")
						{
							// Escape System.String to be used in the regex
		...
		$.validator.addMethod("bankorgiroaccountNL"
			, function(value
				...
				{
					return this.optional(element) ||
						($.validator.methods.bankaccountNL.call(this
							, value
							, element)) ||
						($.validator.methods.giroaccountNL.call(this
							, value
							, element));
				}
		...
		$.validator.addMethod("cifES"
			, function(value
			           , element)
				{
					...
					var letter = value.substring(0
						    , 1)
					    , // [ T ]
					    number = value.substring(1
						    , 8)
					    , // [ P ][ P ][ N ][ N ][ N ][ N ][ N ]
					    control = value.substring(8
						    , 9)
					    , // [ C ]
					    all_sum = 0
					    , even_sum = 0
					...
					control_digit = (10 -
						(all_sum).toString()
						.substr(-1)).toString();
					control_digit = parseInt(control_digit
						                , 10) >
					                9
					                ? "0"
					                : control_digit;
					control_letter = "JABCDEFGHI".substr(control_digit
							, 1)
						.toString();
			...
			, function(value)
				{
					...
					firstCN = parseInt(value.substring(9
							, 10)
						, 10);
					secondCN = parseInt(value.substring(10
							, 11)
						, 10);

					checkResult = function(sum
					                       , cn)
						{
					...
					if (re.test(value))
						{
							...
							mm = parseInt(adata[1]
								, 10);
							aaaa = parseInt(adata[2]
								, 10);
							xdata = new Date(Date.UTC(aaaa
								, mm - 1
								, gg
								, 12
								, 0
							...
							if ((xdata.getUTCFullYear() === aaaa) &&
								...
								(xdata.getUTCDate() === gg)) check = true;
							else check = false;
						}
					else check = false;
		...
		$.validator.addMethod("nieES"
			, function(value
			           , element)
				{
					...
					return validChars.charAt(parseInt(number
								, 10) %
							23) ===
						letter;
				}
		...
		$.validator.addMethod("nifES"
			, function(value
			           , element)
				{
					...
					// Basic format test
					if (!value
						.match("((^[A-Z]{1}[0-9]{7}[A-Z0-9]{1}$|^[T]{1}[A-Z0-9]{8}$)|^[0-9]{8}[A-Z]{1}$)")) return false;

					// Test NIF
					if (/^[0-9]{8}[A-Z]{1}$/.test(value))
						{
							return ("TRWAGMYFPDXBNJZSQVHLCKE".charAt(value.substring(8
										, 0) %
									23) ===
								value.charAt(8));
						}
					...
					if (/^[KLM]{1}/.test(value))
						{
							return (value[8] ===
								"TRWAGMYFPDXBNJZSQVHLCKE".charAt(value.substring(8
										, 1) %
									23));
						}
			...
			, function(value)
				{
					...
					for (var i = 0
						; i < 9
						; i++) intSum += arrSteps[i] * value[i];
					var int2 = intSum % 11;
					var intControlNr = (int2 === 10)
						                   ? 0
						                   : int2;

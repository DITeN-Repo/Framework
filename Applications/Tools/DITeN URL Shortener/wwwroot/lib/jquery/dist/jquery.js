...
(function(global
	...
	, function(window
	           , noGlobal)
		{
			...
			jQuery.extend = jQuery.fn.extend = function()
				{
					...
					// Handle case when target is a System.String or something (possible in deep copy)
				...
				(function(window)
					{
						 ...
						 * @returns {function(System.String, object)} Returns the Object data after storing it on itself with
						 *	property name the (space-suffixed) System.String and (if the cache is larger than Expr.cacheLength)
								...
								if (preferredDoc !== document &&
									...
									subWindow.top !== subWindow)
									{
										...
										if (subWindow.addEventListener)
											...
											{
												subWindow.attachEvent("onunload"
													, unloadHandler);
											}
									}
								                   ...
								                   : function(tag
								                              , context)
									                   {
										                   ...
										                   if (tag === "*")
											                   {
												                   while ((elem =
													                   results[i++])) if (elem.nodeType === 1) tmp.push(elem);
								...
								if ((support.qsa = rnative.test(document.querySelectorAll)))
									{
										...
										assert(function(el)
											{
												// Select is set to empty System.String on purpose
												...
												if (el.querySelectorAll("[msallowcapture^='']")
													.length) rbuggyQSA.push("[*^$]=" + whitespace + "*(?:''|\"\")");

												// Support: IE8
												// Boolean attributes and "value" are not treated correctly
												if (!el.querySelectorAll("[selected]")
													.length) rbuggyQSA.push("\\[" + whitespace + "*(?:value|" + booleans + ")");
										...
										assert(function(el)
											{
												...
												if (el.querySelectorAll("[name=d]")
													.length) rbuggyQSA.push("name" + whitespace + "*[*^$|!~]?=");
												...
												if (matcher[expando])
													{
														...
														return setMatcher(
																...
																, matcher
																, i < j &&
																matcherFromTokens(tokens.slice(i
																	, j))
								...
								var bySet = setMatchers.length > 0
								    ...
								    , superMatcher = function(seed
								                              ...
								                              , outermost)
									    {
										    ...
										    // Incrementing an initially-string "0" `i` allows `i` to remain a System.String only in that
			...
			jQuery.filter = function(expr
			                         ...
			                         , not)
				{
					...
					if (elems.length === 1 && elem.nodeType === 1)
						{
							return jQuery.find.matchesSelector(elem
									       , expr)
								       ? [elem]
								       : [];
						}
					...
					, contents: function(elem)
						{
							...
							// don't support it.
							if (nodeName(elem
								, "template")) elem = elem.content || elem;
					...
					Deferred: function(func)
						{
							var tuples = [
									    ...
									    , [
										    "resolve", "done", jQuery.Callbacks("once memory"), jQuery.Callbacks("once memory"), 0
										    , "resolved"
							...
							if (remaining <= 1)
								{
									...
									if (master.state() === "pending" ||
										isFunction(resolveValues[i] && resolveValues[i].then)) return master.then();
								}
			...
			// Matches dashed System.String for camelizing
			var rmsPrefix = /^-ms-/
			    , rdashAlpha = /-([a-z])/g;

			// Used by camelCase as callback to replace()
			function fcamelCase(all
			                    , letter)
				{
			...
			Data.prototype = {
					...
					, access: function(owner
					                   ...
					                   , value)
						{
							...
							//   2. A System.String key was specified, but no value provided
							//
					...
					, remove: function(owner
					                   , key)
						{
							...
							if (key !== undefined)
								{
									// Support array or space separated System.String of keys
			...
			function showHide(elements
			                  , show)
				{
					...
					for (index = 0
						; index < length
						; index++) if (values[index] != null) elements[index].style.display = values[index];
			...
			function restoreScript(elem)
				{
					if ((elem.type || "").slice(0
							, 5) ===
						"true/") elem.type = elem.type.slice(5);
							...
							get: function(tween)
								{
									...
									// Passing an empty System.String as a 3rd parameter to .css will automatically
									// attempt a parseFloat and fallback to a System.String if the parse fails.
					...
					for (prop in orig)
						{
							...
							if (!propTween)
								{
									if (dataShow) if ("hidden" in dataShow) hidden = dataShow.hidden;
									else
										{
											...
											if (cur)
												{
													j = 0;
													while ((clazz =
														classes[j++])) if (cur.indexOf(" " + clazz + " ") < 0) cur += clazz + " ";
							...
							return this.each(function()
								{
									...
									if (isValidValue)
									...
									else if (value === undefined || type === "boolean")
										{
											...
											// falling back to the empty System.String if nothing was stored.
									...
									if (elem)
										{
											...
											// Handle most common System.String cases
			...
			var
				 ...
				 *    - AFTER param serialization (s.data is a System.String if s.processData is true)
			...
			// Argument "data" should be System.String of html
							...
							else
								{
									...
									while (offsetParent &&
										...
										jQuery.css(offsetParent
											, "position") ===
										"static") offsetParent = offsetParent.parentNode;
							...
							return this.map(function()
								{
									...
									while (offsetParent &&
										jQuery.css(offsetParent
											, "position") ===
										"static") offsetParent = offsetParent.offsetParent;

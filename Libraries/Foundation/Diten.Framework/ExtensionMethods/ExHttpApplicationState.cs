...
namespace Diten
{
	public static class ExHttpApplicationState
	{
		...
		public static void SetValue(this HttpApplicationState httpApplicationState,
		                            string key,
		                            object value)
		{
			SetValue<object>(httpApplicationState,
			                 key,
			                 _return => value);
		}
		...
		/// <param name="key">The key of the item in Application state.</param>
		/// <param name="value">Value thea must be set into the Application state.</param>
		public static void SetValue<T>(this HttpApplicationState httpApplicationState,
		                               string key,
		                               object value)
		{
			SetValue<T>(httpApplicationState,
			            key,
			            _return => value);
		}
		...
		{
			try { httpApplicationState[key] = (T) func.Invoke(func.GetMethodInfo().GetParameters()); }
			catch { httpApplicationState[key] = func.Invoke(func.GetMethodInfo().GetParameters()); }
		}
		...
		public static object GetValue(this HttpApplicationState httpApplicationState,
		                              string key)
		{
			return GetValue<object>(httpApplicationState,
			                        key,
			                        _return => null);
		}
		...
		/// <param name="key">The key of the item in Application state.</param>
		/// <returns><see cref="System.Object" /> type value of the entity in Application state.</returns>
		public static T GetValue<T>(this HttpApplicationState httpApplicationState,
		                            string key)
		{
			return GetValue<T>(httpApplicationState,
			                   key,
			                   _return => null);
		}

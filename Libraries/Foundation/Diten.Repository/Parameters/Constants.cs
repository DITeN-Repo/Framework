...
namespace Diten.Parameters
{
	...
	public partial class Constants
	{
		...
		/// </summary>
		public static IEnumerable<char> DecimalNumbers =>
			Enumerable.Range('0',
			                 10)
			          .Select(x => (char) x)
			          .ToArray();
		...
		private static System.String FrameName
		{
			get
			{
				...
				return _return.StartsWith(SystemParams.Default.GetFrameFunctionExtention,
				                          StringComparison.Ordinal) ? _return.TrimStart(SystemParams.Default.GetFrameFunctionExtention.ToCharArray()) :
				       _return.StartsWith(SystemParams.Default.SetFrameFunctionExtention,
				                          StringComparison.Ordinal) ? _return.TrimStart(SystemParams.Default.SetFrameFunctionExtention.ToCharArray()) :
				       _return;
			}
		}
		...
		public static char[] KeyboardCharacters01 =>
			new Func<char[]>(() =>
			                 {
		...
		public static char[] KeyboardCharacters02 =>
			new Func<char[]>(() =>
			                 {
				                 var _return = KeyboardCharacters01.ToList();

				                 _return.InsertRange(25,
				                                     LowerCaseLetters);
		...
		public static char[] KeyboardCharacters03 =>
			new Func<char[]>(() =>
			                 {
		...
		public static char[] KeyboardCharacters04 =>
			new Func<char[]>(() =>
			                 {
		...
		public static char[] KeyboardCharacters05 =>
			new Func<char[]>(() =>
			                 {
		...
		/// </summary>
		public static IEnumerable<char> LowerCaseLetters =>
			Enumerable.Range('a',
			                 26)
			          .Select(x => (char) x)
			          .ToArray();
		...
		/// </summary>
		public static char[] SexagesimalCharacters =>
			new Func<char[]>(() =>
			                 {
				                 var _return = new List<char>();
		...
		/// </summary>
		public static IEnumerable<char> UpperCaseLetters =>
			Enumerable.Range('A',
			                 26)
			          .Select(x => (char) x)
			          .ToArray();
		...
		/// </summary>
		public static char[] VigesimalCharacters =>
			new Func<char[]>(() =>
			                 {
				                 var _return = new List<char>();

				                 _return.AddRange(DecimalNumbers);
				                 _return.AddRange(Enumerable.Range('A',
				                                                   11)
				                                            .Select(x => (char) x)
				                                            .ToArray());
		...
		public static IEnumerable<string> GetGlobalAssemblyCacheFiles(System.String path)
		{
			...
			foreach (var diChild in directoryInfo.GetDirectories()) files.AddRange(GetGlobalAssemblyCacheFiles(diChild.FullName));

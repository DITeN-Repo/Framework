...
namespace Diten.Collections.Generic
{
	...
	public class Dictionary<T1, T2, T3>: Dictionary<T1, KeyValuePair<T2, T3>>
	{
		...
		public void Remove(T1 key1,
		                   T2 key2)
		{
			foreach (var item in this.Where(k => k.Key.Equals(key1) && k.Value.Key.Equals(key2))) Remove(item.Key);
		}

...
namespace Diten.Collections.Generic
{
	...
	public class List<T>:SCG.List<T>
	{
		...
		public static List<T> ToList(SCG.IEnumerable<T> value)
		{
			return new List<T>().AddRange(value);
		}
		...
		public List<T> Fill()
		{
			return (List<T>)typeof(T).GetMethod(Enum.GetName(Enum.MethodNames.Fill), Type.EmptyTypes)
		...
		public static implicit operator EnumerableQuery<T>(List<T> value)
		{
			return new EnumerableQuery<T>(value.ToList());
		}

...
namespace Diten
{
	...
	public static class ExString
	{
		...
		{
			return new Func<string, string, bool, string>((v, it, ab) =>
			{
				var closingTag = ToClosedTag(v, ab);
				var openingTag = closingTag.Replace(Char.ReservedChars.Slash.ToString(), string.Empty);
		...
		///    .
		/// </returns>
		public static int ToInt(this string value, char[] baseChars)
		{
		...
		public static Collections.Generic.List<Word> ToWords(this string value)
		{
			...
			result.AddRange(RecursiveToWords(value
				.Split(Char.ReservedChars.Space.ToCharArray(), StringSplitOptions.RemoveEmptyEntries)
				.Select(s => new Word(s))));
		...
		public static string ToProtectedString(this string value)
		{
			return Char.UnsafeChars.ToList()
				.Aggregate(string.Empty, (r, c) => value.Replace(c.Ascii, Char.ReservedChars.Underline.ToChar()));
		}

		public static string SingleOutOn(this string value, char character)
		{
			return new Func<string, char, string>((s, c) =>
				{
		...
		/// </returns>
		public static string Remove(this string value, IEnumerable<char> chars)
		{
			return chars.Aggregate(string.Empty, (result, current) => value.Replace(current.ToString(), string.Empty));
		}
		...
		/// </summary>
		/// <param name="value">An <see cref="string" />value.</param>
		public static void Save(this string value)
		{
			new String(value).Save();
		}
		...
		{
			var holder00 = ToUnsafe(Regex.Match(value, "[&]+[\\w]+[;]").Value);
		...
		public static string Translate(this string data, string cultureName)
		{
#if DEBUG

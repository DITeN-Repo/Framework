...
namespace Diten.Pdf
{
	public static class Tools
	{
					...
					for (var currentPageIndex = 1; currentPageIndex <= numberOfPages; currentPageIndex++)
					{
						...
						if (watermark)
						{
							...
							ColumnText.ShowTextAligned(pageStamp.GetOverContent(), Element.ALIGN_CENTER,
								new Phrase("DITeN PDF Tool"), importedPage.Width / 2,
		...
		public static byte[] Merge(IEnumerable<string> sourceFiles,
			...
			Merge(sourceFiles.Select(File.ReadAllBytes).ToList(), watermark);

		public static byte[] Merge(string file1,
			string file2,
			bool watermark = false) =>
			Merge(new Collections.Generic.List<string> {file1, file2}, watermark);
		...
		public static void Merge(string file1,
			...
			bool watermark = false)
		{
			File.WriteAllBytes(exportFile, Merge(file1, file2, watermark));
		}
	}
}
...
RSA.getRandomNumber = randomNUmber => () =>
	{
		var random_num = new Uint8Array(1024 * 16 / 8); // 16384 = number length in bits
...
RSA.generate = keysize =>
	{
		...
		function randomPrime(bits)
			{
				const min = bigInt(6074001000)
					.shiftLeft(bits - 33); // min ≈ √2 × 2^(bits - 1)
				const max = bigInt.one.shiftLeft(bits)
					.minus(1); // max = 2^(bits) - 1
				for (
					;;)
					{
						const p = RSA
							.getRandomNumber; // bigInt.randBetween(min, max);  // WARNING: not a cryptographically secure RNG!

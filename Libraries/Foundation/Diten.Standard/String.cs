...
namespace Diten
{
	...
	[Attributes.Type]
	public class String:Object<String>
	{
		/// <summary>
		///    Constructor.
		/// </summary>
		...
		{
			return new String(value).IsNull();
		}
		...
		public bool IsSafe()
		{
			var range = Char.ControlChars.ToList();
			range.AddRange(Char.ExtendedChars.ToList());
			range.AddRange(Char.UnsafeChars);

			return Value.ToCharArray().All(ch => !Constants.Default.UnsafeChars.Contains(ch));
		}
		...
		{
			return new String(Value.ToSafe());
		}
		...
		{
			return new String(Value.ToUnsafe());
		}
	}
}
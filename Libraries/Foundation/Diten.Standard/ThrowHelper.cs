...
namespace Diten
{
	...
	internal static class ThrowHelper
	{
		...
		{
			return new ArgumentException(string.Format(EX.Argument_AddingDuplicateWithKey, key));
		}
		...
		private static ArgumentOutOfRangeException GetArgumentOutOfRangeException(
			...
			ExceptionResource resource)
		{
			return new ArgumentOutOfRangeException(GetArgumentName(argument)+"["+paramNumber+"]",
		...
		private static InvalidOperationException GetInvalidOperationException_EnumCurrent(
			int index)
		{
			return new InvalidOperationException(index<0
		...
		{
			return new KeyNotFoundException(string.Format(EX.Arg_KeyNotFoundWithKey, key));
		}
		...
		{
			return new ArgumentException(string.Format(EX.Arg_WrongType, key, targetType), nameof(key));
		}
		...
		private static ArgumentException GetWrongValueTypeArgumentException(
			...
			[Nullable(1)] Type targetType)
		{
			return new ArgumentException(string.Format(EX.Arg_WrongType, value, targetType), nameof(value));
		}

...
namespace Diten.Tor
{
	...
	{
		private readonly Dictionary<ConfigurationNames, object> overrides;
		...
		public ClientCreateParams(string path, int controlPort, string controlPassword) : this(path, controlPort,
		...
		public ClientCreateParams(string path, int controlPort, string controlPassword, string configurationFile)
		{
			...
			overrides = new Dictionary<ConfigurationNames, object>();
			this.path = path;
		}

		/// <summary>
		///     Sets the value of a configuration supplied with the process arguments. Setting a configuration value within the
		///     create
		///     parameters overrides any values stored in the configuration file. The control port can also be set using this
		///     method.
		/// </summary>
		/// <param name="configuration">The configuration which should be overridden.</param>
		/// <param name="value">
		///     The value of the configuration. This should align with the type specified in the
		///     <see cref="Configuration" /> class.
		/// </param>
		public void SetConfig(ConfigurationNames configuration, object value)
		{
			var attribute =
				ReflectionHelper.GetAttribute<ConfigurationNames, ConfigurationAssocAttribute>(configuration);

			if (attribute == null)
				return;

			if (value != null && value.GetType() != attribute.Type)
				throw new ArgumentException(@"The value of this configuration should be of type " + attribute.Type,
					"value");
		...
		{
			var compare = obj as ClientCreateParams;
		...
		public override string ToString()
		{
			...
			foreach (var over in overrides)
			{
				...
				var attribute =
					ReflectionHelper.GetAttribute<ConfigurationNames, ConfigurationAssocAttribute>(over.Key);

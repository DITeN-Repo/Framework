...
using Diten.Tor.Events;
using Diten.Tor.Helpers;

#endregion

namespace Diten.Tor.Configuration
{
	/// <summary>
	///     A class containing configuration values supported by the tor application. The configuration values reflect those
	///     available
	...
	public sealed class Configuration : MarshalByRefObject, INotifyPropertyChanged
	{
		...
		{
			this.client = client;
			values = new Dictionary<ConfigurationNames, object>();
		...
		{
			foreach (var v in e.Configurations)
		...
		public bool Save()
		{
			var command = new SaveConfCommand();
		...
		{
			var names = (ConfigurationNames[]) Enum.GetValues(typeof(ConfigurationNames));

			foreach (var name in names)
			{
				var attribute = ReflectionHelper.GetAttribute<ConfigurationNames, ConfigurationAssocAttribute>(name);
		...
		private void SetValue(ConfigurationNames name, object value)
		{
				    ...
				    validation.HasFlag(ConfigurationValidation.PortRange))
				{
					...
					if (validation.HasFlag(ConfigurationValidation.NonZero) && converted == 0)
						throw new ArgumentOutOfRangeException("value",
							"The value of this configuration cannot be zero");
					if (validation.HasFlag(ConfigurationValidation.NonNegative) && converted < 0)
						throw new ArgumentOutOfRangeException("value",
							"The value of this configuration cannot be below zero");
					if (validation.HasFlag(ConfigurationValidation.PortRange) &&
					    (converted <= 0 || short.MaxValue < converted))
						throw new ArgumentOutOfRangeException("value",
							"The value of this configuration must be within a valid port range");
				}
			...
			var command = new SetConfCommand(attribute.Name, ReflectionHelper.Convert(value, typeof(string)) as string);
			var response = command.Dispatch(client);
		}
		...
		internal void SetValueDirect(string name, string value)
		{
			...
			var association = ReflectionHelper.GetEnumerator<ConfigurationNames, ConfigurationAssocAttribute>(attr =>
				attr.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase));
			var attribute = ReflectionHelper.GetAttribute<ConfigurationNames, ConfigurationAssocAttribute>(association);
		...
		internal void Start()
		{
			client.Events.ConfigurationChanged += OnClientConfigurationChanged;
		}

		#region Properties

		/// <summary>
		///     Gets or sets a value indicating whether addresses ending in ".foo.exit" on the SocksPort/TransPort/NATDPort should
		///     be
		///     converted into the target hostname that exit from the node "foo". This is disabled by default as it can open your
		///     request
		///     to modification or manipulation.
		/// </summary>
		public bool AllowDotExit
		{
			get => (bool) GetValue(ConfigurationNames.AllowDotExit);
		...
		{
			get => (bool) GetValue(ConfigurationNames.AllowNonRFC953HostNames);
		...
		{
			get => (bool) GetValue(ConfigurationNames.AvoidDiskWrites);
		...
		{
			get => (Bytes) GetValue(ConfigurationNames.BandwidthRate);
		...
		{
			get => (int) GetValue(ConfigurationNames.CircuitBuildTimeout);
		...
		{
			get => (int) GetValue(ConfigurationNames.CircuitIdleTimeout);
		...
		{
			get => (bool) GetValue(ConfigurationNames.ClientPreferIPv6ORPort);
		...
		{
			get => (bool) GetValue(ConfigurationNames.ClientUseIPv6);
		...
		{
			get => (bool) GetValue(ConfigurationNames.ConstrainedSockets);
			set => SetValue(ConfigurationNames.ConstrainedSockets, value);
		}

		/// <summary>
		///     Gets or sets the size of the socket buffers when the <see cref="ConstrainedSockets" /> setting is enabled. This
		...
		{
			get => (Bytes) GetValue(ConfigurationNames.ConstrainedSockSize);
		...
		{
			get => (int) GetValue(ConfigurationNames.ControlPort);
			set => SetValue(ConfigurationNames.ControlPort, value);
		}

		/// <summary>
		///     Gets or sets a value indicating whether the Tor application shouldn't listen for accept any connections other than
		///     control connections.
		/// </summary>
		public bool DisableNetwork
		{
			get => (bool) GetValue(ConfigurationNames.DisableNetwork);
			set => SetValue(ConfigurationNames.DisableNetwork, value);
		}

		/// <summary>
		///     Gets or sets a value indicating whether Tor should not put two servers whose IP addresses are "too close" on the
		///     same circuit. Currently,
		///     two addresses are considered "too close" if they line in the same /16 range.
		/// </summary>
		public bool EnforceDistinctSubnets
		{
			get => (bool) GetValue(ConfigurationNames.EnforceDistinctSubnets);
		...
		{
			get => (bool) GetValue(ConfigurationNames.ExcludeSingleHopRelays);
		...
		{
			get => (bool) GetValue(ConfigurationNames.FascistFirewall);
			set => SetValue(ConfigurationNames.FascistFirewall, value);
		}

		/// <summary>
		///     Gets or sets a value indicating whether Tor will not use the public key step for the first hop of creating
		///     circuits. A value
		///     of <c>Auto.Auto</c> indicates that the Tor network will take advice from the authorities in the latest consensus on
		///     using the feature.
		///     Disabling this feature may cause the circuit building to execute slower.
		/// </summary>
		public Auto FastFirstHopPK
		{
			get => (Auto) GetValue(ConfigurationNames.FastFirstHopPK);
		...
		{
			get => GetValue(ConfigurationNames.GeoIPFile) as string;
		...
		{
			get => GetValue(ConfigurationNames.GeoIPv6File) as string;
		...
		{
			get => (bool) GetValue(ConfigurationNames.HardwareAcceleration);
		...
		{
			get => (string) GetValue(ConfigurationNames.HashedControlPassword);
			set => SetValue(ConfigurationNames.HashedControlPassword, value);
		}

		/// <summary>
		///     Gets or sets the proxy host through which Tor will make all directory requests. This causes Tor to connect through
		///     the proxy
		///     host address and port number, rather than communicating with the directory directly. A value of <c>Host.Null</c>
		///     indicates that
		///     no proxy should be used.
		/// </summary>
		public Host HTTPProxy
		{
			get => (Host) GetValue(ConfigurationNames.HTTPProxy);
		...
		{
			get => (HostAuth) GetValue(ConfigurationNames.HTTPProxyAuthenticator);
			set => SetValue(ConfigurationNames.HTTPProxyAuthenticator, value);
		}

		/// <summary>
		///     Gets or sets the proxy host through which Tor will make all SSL directory requests. This causes Tor to connect
		///     through the proxy
		...
		{
			get => (Host) GetValue(ConfigurationNames.HTTPSProxy);
		...
		{
			get => (HostAuth) GetValue(ConfigurationNames.HTTPSProxyAuthenticator);
			set => SetValue(ConfigurationNames.HTTPSProxyAuthenticator, value);
		}

		/// <summary>
		///     Gets or sets the interval (in seconds) between packets sent from Tor containing padded keep-alive values. This is
		///     used when
		///     running behind a firewall to prevent open connections from being terminated prematurely.
		/// </summary>
		public int KeepAlivePeriod
		{
			get => (int) GetValue(ConfigurationNames.KeepAlivePeriod);
		...
		{
			get => (bool) GetValue(ConfigurationNames.LearnCircuitBuildTimeout);
			set => SetValue(ConfigurationNames.LearnCircuitBuildTimeout, value);
		}

		/// <summary>
		///     Gets or sets the maximum time (in seconds) in which a circuit can be re-used after the first use. This will allow
		///     existing
		///     circuits to be re-used, but will not attach new streams onto these circuits.
		/// </summary>
		public int MaxCircuitDirtiness
		{
			get => (int) GetValue(ConfigurationNames.MaxCircuitDirtiness);
			set => SetValue(ConfigurationNames.MaxCircuitDirtiness, value);
		}

		/// <summary>
		///     Gets or sets the maximum number of circuits to be pending at a time when handling client streams. A circuit is
		///     pending
		///     if Tor has begun constructing it, but it has not yet been completely constructed.
		/// </summary>
		public int MaxClientCircuitsPending
		{
			get => (int) GetValue(ConfigurationNames.MaxClientCircuitsPending);
		...
		{
			get => (int) GetValue(ConfigurationNames.NewCircuitPeriod);
			set => SetValue(ConfigurationNames.NewCircuitPeriod, value);
		}

		/// <summary>
		///     Gets or sets a value indicating whether Tor, when using an exit node that supports the feature, should try to
		///     optimistically send data
		///     to the exit node without waiting for the exit node to report whether the connection was successful. A value of
		///     <c>Auto.Auto</c>
		...
		{
			get => (Auto) GetValue(ConfigurationNames.OptimisticData);
			set => SetValue(ConfigurationNames.OptimisticData, value);
		}

		/// <summary>
		///     Gets or sets a value indicating whether Tor should reject connections which use unsafe variants of the SOCKS
		///     protocol.
		/// </summary>
		public bool SafeSocks
		{
			get => (bool) GetValue(ConfigurationNames.SafeSocks);
			set => SetValue(ConfigurationNames.SafeSocks, value);
		}

		/// <summary>
		///     Gets or sets the port number that the Tor application will listen for SOCKS connections upon. A value of zero
		///     indicates that
		///     the Tor application should not listen for SOCKS connections.
		/// </summary>
		public int SocksPort
		{
			get => (int) GetValue(ConfigurationNames.SocksPort);
		...
		{
			get => (int) GetValue(ConfigurationNames.SocksTimeout);
			set => SetValue(ConfigurationNames.SocksTimeout, value);
		}

		/// <summary>
		///     Gets or sets a value indicating whether Tor should use micro-descriptors when building circuits. Micro-descriptors
		///     are
		///     a smaller version of information needed when building circuits, allowing the client to use less directory
		///     information.
		/// </summary>
		public Auto UseMicroDescriptors
		{
			get => (Auto) GetValue(ConfigurationNames.UseMicroDescriptors);

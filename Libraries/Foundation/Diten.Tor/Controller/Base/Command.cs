...
{
	/// <summary>
	///     A class containing the base methods and properties for a command which will be executed across a control
	///     connection,
	...
	internal abstract class Command<T> where T : Response
	{
		...
		public T Dispatch(Client client)
		{
				...
				using (var connection = new Connection(client))
				{
					if (!connection.Connect())
						throw new TorException(
							"A command could not be dispatched to a client because the command failed to connect to the control port");

					if (!connection.Authenticate(client.GetControlPassword()))
						throw new TorException(
							"A command could not be dispatched to a client because the control could not be authenticated");
			...
			{
				throw new TorException("A command could not be dispatched to a client because an error occurred",
					exception);
			}
		}

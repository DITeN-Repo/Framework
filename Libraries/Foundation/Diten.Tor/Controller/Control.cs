...
namespace Diten.Tor.Controller
{
	...
	public sealed class Control : MarshalByRefObject
	{
		...
		public bool CloseCircuit(Circuit circuit)
		{
			...
			var command = new CloseCircuitCommand(circuit);
		...
		public bool CloseStream(Stream stream, StreamReason reason)
		{
			...
			if (reason == StreamReason.None || reason == StreamReason.End || reason == StreamReason.PrivateAddr)
				throw new ArgumentOutOfRangeException("reason",
					"The reason for closure cannot be None, End or PrivateAddr");

			var command = new CloseStreamCommand(stream, reason);
			var response = command.Dispatch(client);
			return response.Success;
		}
		...
		{
			var command = new CreateCircuitCommand();
			var response = command.Dispatch(client);
		...
		{
			var command = new CreateCircuitCommand(routers);
			var response = command.Dispatch(client);
			return response.Success && response.CircuitID >= 0;
		}
		...
		public bool ExtendCircuit(Circuit circuit, params string[] routers)
		{
			...
			if (circuit.ID == 0)
				throw new ArgumentException("The circuit has an invalid ID", "circuit");
			if (routers.Length == 0)
				throw new ArgumentOutOfRangeException("routers",
					"At least one router should be supplied with the extend");

			var command = new ExtendCircuitCommand(circuit);

...
namespace Diten.Tor.Helpers
{
	...
	internal static class ReflectionHelper
	{
		...
		{
			if (value == null)
				return destinationType.IsPrimitive || destinationType.IsValueType
					? Activator.CreateInstance(destinationType)
					: null;

			if (destinationType.IsEnum)
				return GetEnumerator<DescriptionAttribute>(destinationType,
					attribute =>
						attribute.Description.Equals(value.ToString(), StringComparison.CurrentCultureIgnoreCase));
		...
		public static TAttribute GetAttribute<TEnum, TAttribute>(TEnum value) where TAttribute : Attribute
		{
			var member = typeof(TEnum)
				.GetMember(System.Convert.ToString(value), BindingFlags.Public | BindingFlags.Static).FirstOrDefault();
		...
		public static object GetEnumerator<TAttribute>(Type enumType, Func<TAttribute, bool> selector)
			where TAttribute : Attribute
		{
			...
			{
				var attribute = Attribute.GetCustomAttribute(member, typeof(TAttribute)) as TAttribute;
		...
		/// <param name="selector">The selector to execute against the attribute.</param>
		/// <returns>A <typeparamref name="TEnum" /> value matching the selector pattern.</returns>
		public static TEnum GetEnumerator<TEnum, TAttribute>(Func<TAttribute, bool> selector)
			where TAttribute : Attribute
		{
		...
		{
			var values = new List<TAttribute>();
			MemberInfo[] members = typeof(TEnum).GetFields(BindingFlags.Public | BindingFlags.Static);
		...
		public static List<TValue> GetEnumeratorAttributes<TEnum, TAttribute, TValue>(Func<TAttribute, TValue> selector)
			where TAttribute : Attribute
		{
			var attributes = GetEnumeratorAttributes<TEnum, TAttribute>();

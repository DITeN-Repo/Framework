...
namespace Diten.Tor.Net
{
	...
	internal sealed class Socks5Processor
	{
		...
		public Socks5AsyncResult BeginProcessing(IPEndPoint remoteEP, ProcessorCallback callback)
		{
			...
			socket.BeginConnectInternal(new IPEndPoint(IPAddress.Parse(socket.ProxyAddress), socket.ProxyPort),
		...
		private void OnSocketReceiveHandshake(IAsyncResult ar)
		{
			try
			{
				...
				if (endPoint != null)
				{
					...
					var port = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short) endPoint.Port));
				...
				else
				{
					...
					var address = Encoding.ASCII.GetBytes(endAddress);
					var port = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short) endPort));

	...
	internal sealed class ForwardSocket : Socket
	{
		...
		private void OnDnsGetHostEntry(IAsyncResult ar)
		{
			try
			{
				...
				var state = (DNSResolveState) ar.AsyncState;
		...
		///         version="1" Unrestricted="true" />
		///     <IPermission
		///         class="System.Net.SocketPermission, System, Version=2.0.3600.0, Culture=neutral, PublicKeyToken=b77a5c561934e089"
		///         version="1" Unrestricted="true" />
		/// </PermissionSet>
		public new IAsyncResult BeginConnect(EndPoint remoteEP, AsyncCallback callback, object state)
		{
			...
			if (ProxyAddress == null || ProxyPort == -1) return base.BeginConnect(remoteEP, callback, state);
		...
		public new IAsyncResult BeginConnect(string host, int port, AsyncCallback callback, object state)
		{
			...
			if (ProxyAddress == null || ProxyPort == -1) return BeginDNSResolve(host, port, callback, state);
		...
		///     An <see cref="T:System.IAsyncResult" /> that stores state information and any user defined

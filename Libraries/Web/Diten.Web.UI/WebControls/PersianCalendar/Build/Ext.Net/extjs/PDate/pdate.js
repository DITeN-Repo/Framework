	  ...
	  */
		...
		createParser: (function() {
			...
			return function(format) {
				...
				utilPDate.parseFunctions[format] =
		 ...
		 */
			...
			D: function() {
				for (var a = [], i = 0;
					i < 7;
					a.push(utilPDate.getShortDayName(i)), ++i); // get localised short day names
				return {
					g: 0,
					c: null,
					s: `(?:${a.join("|")})`
			...
			M: function() {
				for (var a = [], i = 0;
					i < 12;
					a.push(utilPDate.getShortMonthName(i)), ++i); // get localised short month names
				return Ext.applyIf({
						s: `(${a.join("|")})`
					},
	  ...
	  */
		add: function(date, interval, value) {
			...
			case Ext.Date.MONTH:
				...
				if (day > 29) {
					day = Math.min(day,
						Ext.PDate.getLastDateOfMonth(Ext.PDate.add(Ext.PDate.getFirstDateOfMonth(d),
							Ext.Date.MONTH,
							value)).getDate());
				}

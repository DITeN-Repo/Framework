...
Ext.define("Ext.picker.PDate",
	{
		...
		initEvents: function() {
			...
			me.keyNav = new Ext.util.KeyNav(pickerField ? pickerField.inputEl : me.eventEl,
				Ext.apply({
						scope: me,
						...
						left: function(e) {
							if (e.ctrlKey) {
								...
								me.update(eDate.add(me.activeDate, day, (me.inheritedState.Page.Rtl ? -1 : 1) * -1));
							}
						},
						...
						space: function() {
							...
							const startValue = me.startValue;
		...
		onOkClick: function(picker, value) {
			...
			const month = value[0];
			const year = value[1];
			const gd = Ext.PDate.PersianToGregorian(year, month, Ext.PDate.getDate(me.getActive()));
			const date = new Date(gd[0], month == 0 ? gd[1] - 1 : gd[1], gd[2]);
		...
		showPrevMonth: function(e) {
			return this.update(Ext.PDate.add(this.activeDate,
				Ext.Date.MONTH,
				(this.inheritedState.Page.Rtl ? -1 : 1) * -1));
		},
		...
		update: function(date, forceRefresh) {
			...
			if (me.rendered) {
				...
				if (!forceRefresh &&
					active &&
					me.el &&
					Ext.PDate.getMonth(active) == Ext.PDate.getMonth(date) &&
					Ext.PDate.getFullYear(active) == Ext.PDate.getFullYear(date)) {
